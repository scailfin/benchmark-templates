"""Utilities for preparing and running workflow templates.""""
...
def upload_files(template, arguments, loader):
    """Upload local files that are listed in the inputs/files section of
    ...
    """
    if 'inputs' in template.workflow_spec:
        if 'files' in template.workflow_spec['inputs']:
            for val in template.workflow_spec['inputs']['files']:
